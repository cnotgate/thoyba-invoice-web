...
async function fixTotalValues() {
	try {
		...
		const currentTotal = suspiciousInvoices.reduce(
			(sum, inv) => sum + parseFloat(inv.total as string),
			0
		);

		console.log(`Current total of suspicious invoices: Rp ${currentTotal.toLocaleString('id-ID')}`);
		console.log(
			`After fix (÷ 100): Rp ${(currentTotal / 100).toLocaleString('id-ID')}`
		);
		console.log();
		...
		const verifyInvoices = await db
			.select()
			.from(invoices)
			.where(sql`${invoices.id} IN (${sql.raw(suspiciousInvoices.slice(0, 5).map((i) => i.id).join(','))})`);
